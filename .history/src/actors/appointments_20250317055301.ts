  ...
  states: {
    ...
    canceling: {
      invoke: {
          ...
          async ({ input }: { input: CancelInput }): Promise<CancelResult> => {
            return new Promise((resolve, reject) => {
              setTimeout(() => {
                ...
                // Simulate 90% success rate for cancellation
                if (Math.random() > 0.1) {
                  resolve(result);
                } else {
                  reject(result);
        ...
        onError: {
          target: 'idle', // Return to idle on failure (could add a 'failed' state if needed)
          actions: assign({
            appointments: ({ context, event }) => {
              const errorEvent = event as unknown as { data: CancelResult };
              // Optionally log failure; here we just return unchanged for simplicity
