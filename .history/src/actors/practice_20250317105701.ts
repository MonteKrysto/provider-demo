  ...
  states: {
    loading: {
      invoke: {
        ...
        onDone: {
          ...
          guard: (_, event) => event !== undefined && event.data !== null, // Ensure event and data are valid
        },
        onError: {
          target: 'running', // Proceed even on error for now
          actions: assign({
            isLoading: false,
          }),
        },
    ...
    running: {
      entry: [
        ({ context, self }) => {
          console.log('Starting all actors');
          const data = self.getSnapshot().event?.data; // Access data from invoke result
          if (data) {
