  ...
  states: {
    idle: {
      on: {
        ...
        INITIALIZE: {
          actions: assign<PatientContext, { type: 'INITIALIZE'; data: Patient[] }, { type: 'INITIALIZE'; data: Patient[] }, EventObject, EventObject>({
            patients: (_, event) => event.data || [],
          }),
        },
      },
    },
          ...
          actions: assign({
            patients: ({ context }) => {
              if (context.activePatient) {
                ...
                ).filter((p): p is Patient => p !== null); // Filter out null values
              }
              return context.patients;
            },
            activePatient: null,
