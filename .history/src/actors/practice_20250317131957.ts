      ...
      invoke: {
        ...
        onDone: {
          ...
          actions: [
            ...
            ({ context, event }) => {
              // Ensure event.data exists (it should, per fromPromise)
              const data = event.data as { patients: any; notes: any; invoices: any; appointments: any };
          ...
          guard: ({ event }) => event.data !== null && event.data !== undefined, // Explicit check
        },
        onError: {
          target: 'running',
          actions: assign({ isLoading: false }),
