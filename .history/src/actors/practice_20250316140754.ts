...
export const practiceMachine = createMachine({
  ...
  initial: 'running',
  context: {
    patientRef: null as any, // Set by spawn
    notesRef: null as any,   // Set by spawn
    activePatient: null,
    modal: {
      isOpen: false,
      content: null,
  ...
  states: {
    running: {
      entry: assign({
        patientRef: () => spawn(patientMachine, { name: 'patient', sync: true, inspect: inspector?.inspect }),
        notesRef: () => spawn(notesMachine, { name: 'notes', sync: true, inspect: inspector?.inspect }),
      }),
      on: {
        EDIT_PATIENT: {
