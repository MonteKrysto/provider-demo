    ...
    loading: {
      invoke: {
        src: fromPromise(async () => {
          console.log('Starting fetch from http://localhost:300/api/data');
          ...
          try {
            const response = await fetch('http://localhost:300/api/data', {
        ...
        onDone: {
          ...
          actions: [
            ...
            ({ context, event }) => {
              const data = event.output as { patients: any; notes: any; invoices: any; appointments: any }; // Changed from event.data to event.output
        ...
        SAVE_DRAFT: {
          actions: [
            ({ context, event }) => {
              context.notesRef.send(event);
              console.log('SAVE_DRAFT event sent', event);
              fetch('http://localhost:300/api/update', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ type: 'SAVE_DRAFT', payload: event }),
              })
