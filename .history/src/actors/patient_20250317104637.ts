...
import { createMachine, assign } from 'xstate';
import { Patient, Medication, PracticeEvent } from '../types';
  ...
  states: {
    idle: {
      on: {
        ...
        INITIALIZE: {
          actions: assign({
            patients: ({ context }, event: Extract<PracticeEvent, { type: 'INITIALIZE' }>) => {
              if ('data' in event && Array.isArray(event.data)) {
                return event.data as Patient[];
              }
              return context.patients; // Fallback to current state if event is invalid
            },
          }),
        },
      },
