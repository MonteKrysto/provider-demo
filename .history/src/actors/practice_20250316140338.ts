import { createMachine, assign,  } from 'xstate'; // spawn is an action creator
...
export const practiceMachine = createMachine({
  ...
  initial: 'running',
  context: {
    patientRef: null as any, // TypeScript workaround, set by spawn
    notesRef: null as any,   // TypeScript workaround, set by spawn
    activePatient: null,
    modal: {
      isOpen: false,
      content: null,
      ...
      entry: assign({
        patientRef: () =>
          spawn(patientMachine, {
            name: 'patient',
            sync: true,
            inspect: inspector?.inspect, // Optional chaining for dev-only
          }),
        notesRef: () =>
          spawn(notesMachine, {
            name: 'notes',
