    ...
    processing: {
      invoke: {
        ...
        onDone: {
          target: 'idle',
          actions: assign({
            invoices: ({ context, event }: { context: BillingContext; event: { data: PaymentResult } }) => {
              return context.invoices.map((inv) =>
                inv.id === event.data.invoiceId ? { ...inv, status: 'paid' as const } : inv
              );
            },
          }),
        },
        onError: {
          ...
          actions: assign({
            invoices: ({ context, event }: { context: BillingContext; event: { data: PaymentResult } }) => {
              return context.invoices.map((inv) =>
                inv.id === event.data.invoiceId ? { ...inv, status: 'disputed' as const } : inv
              );
            },
          }),
        },
