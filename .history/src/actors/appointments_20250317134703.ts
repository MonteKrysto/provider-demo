          ...
          actions: assign({
            appointments: ({ context, event }) => {
              ...
              if ('output' in event && event.output) {
                return event.output as Appointment[] || [];
              }
    ...
    canceling: {
      invoke: {
        ...
        onDone: {
          ...
          actions: assign({
            appointments: ({ context, event }) => {
              const { output } = event as { output: CancelResult }; // Use output for consistency
              return context.appointments.map((appt) =>
                appt.id === output.appointmentId ? { ...appt, status: 'canceled' as const } : appt
              );
            },
          }),
        },
        onError: {
          ...
          actions: assign({
            appointments: ({ context, event }) => {
              const { output } = event as unknown as { output: CancelResult }; // Two-step assertion
              return context.appointments.map((appt) =>
                appt.id === output.appointmentId ? { ...appt, status: 'scheduled' as const } : appt
              );
            },
          }),
        },
