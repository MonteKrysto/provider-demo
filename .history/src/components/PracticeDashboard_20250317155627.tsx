...
export function PracticeDashboard() {
    ...
    return (
      <div className="p-4 min-h-screen bg-gray-100">
        <Card className="p-6 max-w-lg mx-auto bg-white shadow-md rounded-lg">
          ...
          <Button onClick={() => window.location.reload()} className="w-full sm:w-auto bg-red-500 hover:bg-red-600 text-white rounded-md">
            Retry
          </Button>
        </Card>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6" style={{ background: 'var(--background)' }}>
      <div className="max-w-2xl mx-auto">
        <Card className="p-6 bg-white shadow-lg rounded-lg border border-gray-200" style={{ background: 'var(--card)' }}>
          <h1 className="text-2xl font-bold mb-6 text-gray-800 border-b-2 border-gray-200 pb-2" style={{ color: 'var(--card-foreground)' }}>
            Practice Dashboard
          </h1>

          {/* Patient List Section */}
          <section className="mb-8 bg-gray-50 p-4 rounded-md shadow-sm" style={{ background: 'var(--muted)' }}>
            <h2 className="text-lg font-semibold mb-4 text-gray-700" style={{ color: 'var(--muted-foreground)' }}>Patients</h2>
            <PatientList
              patients={patientState.context.patients}
              onSelect={(patient) => send({ type: 'EDIT_PATIENT', data: { id: patient.id } })}
            />
            <Dialog open={isAddPatientModalOpen} onOpenChange={setIsAddPatientModalOpen}>
              <DialogTrigger asChild>
                <Button className="mt-4 w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white rounded-md">
                  Add New Patient
                </Button>
              </DialogTrigger>
          ...
          {/* Notes Section */}
          <section className="mb-8 bg-gray-50 p-4 rounded-md shadow-sm" style={{ background: 'var(--muted)' }}>
            <h2 className="text-lg font-semibold mb-4 text-gray-700" style={{ color: 'var(--muted-foreground)' }}>Notes</h2>
            <NotesEditor
              note={notesState.context.currentNote}
              notes={notesState.context.notes}
              onSave={(content) => send({ type: 'SAVE_DRAFT', content, activePatient: mockPatient })}
          ...
          <section className="mb-8 bg-gray-50 p-4 rounded-md shadow-sm" style={{ background: 'var(--muted)' }}>
            <h2 className="text-lg font-semibold mb-4 text-gray-700" style={{ color: 'var(--muted-foreground)' }}>Billing Status</h2>
            <BillingStatus invoices={billingState.context.invoices} />
            <div className="flex gap-3 mt-4 flex-col sm:flex-row">
              ...
              <Button
                onClick={() => send({ type: 'PROCESS_PAYMENT', invoiceId: billingState.context.invoices[0]?.id || '' })}
                disabled={!billingState.context.invoices.length}
                className="w-full sm:w-auto mt-2 sm:mt-0 bg-blue-600 hover:bg-blue-700 text-white disabled:bg-gray-400 disabled:text-gray-600 rounded-md"
              >
                Process Payment
              </Button>
            </div>
          ...
          {/* Calendar Section */}
          <section className="mb-8 bg-gray-50 p-4 rounded-md shadow-sm" style={{ background: 'var(--muted)' }}>
            <h2 className="text-lg font-semibold mb-4 text-gray-700" style={{ color: 'var(--muted-foreground)' }}>Appointments</h2>
            <Calendar appointments={appointmentsState.context.appointments} />
            <div className="flex gap-3 mt-4 flex-col sm:flex-row">
              <Button
                onClick={() =>
          ...
          <Button
            onClick={() => send({ type: 'OPEN_MODAL', content: 'patientDetails' })}
            disabled={!patientState.matches('idle')}
            className="w-full mt-4 bg-blue-600 hover:bg-blue-700 text-white rounded-md"
          >
            View Patient Details
          </Button>
        </Card>
