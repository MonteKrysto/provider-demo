...
//                   if (data.success && data.note) {
//                     context.notesRef.send({
//                       type: 'UPDATE_NOTE_WITH_RESPONSE',
//                       note: data.note,
//                     });
//                   } else {
...
//           actions: [
//             ({ context, event }: { context: PracticeContext; event: PracticeEvent }) => {
//               const cancelEvent = event as Extract<PracticeEvent, { type: 'CANCEL' }>;
//               console.log('CANCEL event sent with ID:', cancelEvent.appointment.id);
//               context.appointmentsRef.send(event);
//               fetch('http://localhost:3000/api/update', {
//                 method: 'POST',
//                 headers: { 'Content-Type': 'application/json' },
